...
def prix_eth():
    try:
        ...
        prix_eth = eth.get_eth_last_price()
        eth_btc = float(round(prix_eth["ethbtc"]), 2)
        eth_usd = float(prix_eth["ethusd"])
...
def gas_oracle():
    timestamp = int(time.time())
    requete_gas = eth.get_gas_oracle()  # requete globale
    dernier_block = requete_gas["LastBlock"]
    safe_gas_price = requete_gas[
        "SafeGasPrice"
    ]  # en gwei : prix le plus bas proposé, attendre + pour payer moins de gas fees

    propose_gas_price = requete_gas["ProposeGasPrice"]  # en gwei : frais de gas marché

    fast_gas_price = requete_gas["FastGasPrice"]
    # en gwei : frais de gas élevés pour aller + vite
    suggest_base_fee = requete_gas["suggestBaseFee"]
    ...
    gas_used_ratio = requete_gas["gasUsedRatio"]
